...
import { generateEditedImage, generateFilteredImage, generateAdjustedImage, generateSwappedImage } from './services/geminiService';
...
type Tab = 'retouch' | 'swap' | 'adjust' | 'filters' | 'crop';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const loadDefaultImage = async () => {
      try {
        const response = await fetch('image.png'); // Use the provided image
        if (!response.ok) {
          // No default image found, proceed to start screen. Not an error.
          return;
        }
        const blob = await response.blob();
        const defaultImageFile = new File([blob], 'default-image.png', { type: blob.type });
        handleImageUpload(defaultImageFile);
      } catch (err) {
  ...
  const handleApplySwap = useCallback(async (clothingSourceImage: File, itemsToSwap: string[]) => {
    if (!currentImage) {
      setError('No base image loaded to apply a swap to.');
      return;
    }
     if (!clothingSourceImage || itemsToSwap.length === 0) {
        setError('No clothing image or items selected for the swap.');
        return;
    }
    ...
    try {
        const swappedImageUrl = await generateSwappedImage(currentImage, clothingSourceImage, itemsToSwap);
        const newImageFile = dataURLtoFile(swappedImageUrl, `swapped-${Date.now()}.png`);
        addImageToHistory(newImageFile);
    } catch (err) {
        const errorMessage = err instanceof Error ? err.message : 'An unknown error occurred.';
        setError(`Failed to apply the clothing swap. ${errorMessage}`);
        console.error(err);
    } finally {
        setIsLoading(false);
    }
    ...
    if (isInitialLoading) {
      return (
        <div className="flex flex-col items-center justify-center gap-4 text-center animate-fade-in">
          <Spinner />
          <p className="text-gray-300 text-lg mt-4">Preparing your studio...</p>
        </div>
      );
    }
    ...
    return (
      <div className="w-full max-w-4xl mx-auto flex flex-col items-center gap-6 animate-fade-in">
        ...
        <div className="w-full bg-gray-800/80 border border-gray-700/80 rounded-lg p-2 flex items-center justify-center gap-2 backdrop-blur-sm">
            {(['retouch', 'swap', 'crop', 'adjust', 'filters'] as Tab[]).map(tab => (
                 <button
                    key={tab}
        ...
        <div className="w-full">
            ...
            {activeTab === 'swap' && <SwapPanel onApplySwap={handleApplySwap} isLoading={isLoading} />}
